...
use tracing::{info, instrument, callsite::register};

use crate::net::rest::routes::main_entry;
...
pub async fn startup(addr: String, register: Sender<String>) -> JoinHandle<()> {
    ...
    tokio::spawn(async move {
        ...
        axum::Server::bind(&addr.parse().unwrap()).serve(main_entry_route.into_make_service()).await.unwrap();
    })
}
